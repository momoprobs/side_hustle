...
@objc class ServiceManager : NSObject {
    ...
    func sendContent(content : String) {
        NSLog("%@", "sendContent: \(content)")
        
        if session.connectedPeers.count > 0 {
            var error : NSError?
            do {
                try self.session.sendData(content.dataUsingEncoding(NSUTF8StringEncoding, allowLossyConversion: false)!, toPeers: session.connectedPeers, withMode: MCSessionSendDataMode.Reliable)
...
extension ServiceManager : MCSessionDelegate {
    ...
    func session(session: MCSession, didReceiveData data: NSData, fromPeer peerID: MCPeerID) {
        ...
        self.delegate?.profileChanged(self, contentString: str)
    }
